...
function CoordinateAxes() {
  ...
  return (
    <group>
      ...
      {/* Labels */}
      <Text ref={xAxisLabelRef} fontSize={0.3} color="#ef4444" fontWeight={"bold"}>
        X (Front)
      </Text>
      <Text ref={zAxisLabelRef} fontSize={0.3} color="blue" fontWeight={"bold"}>
        Z (Up)
      </Text>
      <Text ref={yAxisLabelRef} fontSize={0.3} color="#22c55e" fontWeight={"bold"}>
        Y (Left)
      </Text>
    </group>
  );
...
export function Transform3DVisualizer({
  ...
  const [transform, setTransform] = useState<Transform3DState>(initialTransform);
  const defaultCameraPose: [x: number, y: number, z: number] = [3, 3, -3];

  const updateRotation = (axis: keyof typeof transform.rotation, value: number) => {
    const newTransform = {
      ...transform,
  ...
  const updateTranslation = (axis: keyof typeof transform.translation, value: number) => {
    const newTransform = {
      ...transform,
  ...
  const resetTransform = () => {
    ...
    setTransform(resetValues)
    onTransformChange?.(resetValues)
  }
  ...
  const Scene = () => (
    ...
    >
      ...
      <TransformableObject transform={transform} color={color}>
        {children}
      </TransformableObject>

      {showAxes && <CoordinateAxes />}
      {showGrid && <Grid args={[20, 20, 20]} sectionColor={"black"} cellColor={"rgba(256,256, 256, 0.5)"} />}
