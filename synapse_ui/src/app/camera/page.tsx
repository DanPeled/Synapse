...
import { RefreshCw, Camera, Download, ChevronUp, ChevronDown } from "lucide-react"
  ...
  resolutionResults: [
    {
      ...
      distortionCoefficients: { k1: -0.2841, k2: 0.1024, p1: -0.0012, p2: 0.0008, k3: -0.0234 }
    },
    {
      resolution: "1280x720",
      successful: 12,
      ...
      distortionCoefficients: { k1: -0.2856, k2: 0.1031, p1: -0.0011, p2: 0.0009, k3: -0.0241 }
    },
    {
      resolution: "640x480",
      successful: 16,
      ...
      distortionCoefficients: { k1: -0.2923, k2: 0.1087, p1: -0.0015, p2: 0.0012, k3: -0.0267 }
    }
  ]
}
...
function CalibrationHeader({ timestamp, collapsed, onToggle }: { timestamp: string, collapsed: boolean, onToggle: () => void }) {
  return (
    <div className="flex items-center justify-between" style={{ color: teamColor }}>
      <div className="flex items-center gap-3">
        <Camera className="w-8 h-8" />
        <div>
          <h1 className="text-lg font-bold">Camera Intrinsic Calibration</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            Determines the internal parameters of the camera lens
            to accurately map data from a 2D image into 3D space.
          </p>
        </div>
      </div>
      <Row className="items-center gap-3">
        ...
        >
          {collapsed ? <ChevronDown className="w-5 h-5" color={teamColor} /> : <ChevronUp className="w-5 h-5" color={teamColor} />}
        </Button>
        {!collapsed && (
          <>
            <Button size="sm" style={{ color: teamColor }}>
  ...
  return (
    <Card className="bg-zinc-900 border-gray-700" style={{ color: teamColor }}>
      <CardHeader className="pb-3">
        ...
        <CardDescription>View calibration parameters for specific resolution</CardDescription>
      </CardHeader>
      <CardContent className="pt-0">
        <Row className="items-center gap-2">
          {resolutions.map((result, index) => (
            ...
            >
              {result.resolution}
              <Badge className={`ml-2 ${getStatusColor(result.status)}`} variant="secondary">
                {result.status}
              </Badge>
            </Button>
          ))}
...
function CameraMatrixCard({ data }: { data: typeof calibrationData.resolutionResults[number] }) {
  return (
    <Card className="bg-zinc-900 border-gray-700" style={{ color: teamColor }}>
      <CardHeader>
        <CardTitle>Camera Matrix (K) - {data.resolution}</CardTitle>
        <CardDescription>Intrinsic camera parameters in pixel coordinates</CardDescription>
      </CardHeader>
      <CardContent>
        ...
        <div className="text-xs space-y-1">
          <p><strong>fx, fy:</strong> Focal lengths</p>
          <p><strong>cx, cy:</strong> Principal point</p>
        </div>
      </CardContent>
    </Card>
  )
...
function MatrixCell({ label, value, color }: { label: string, value: number, color?: string }) {
  ...
  return (
    <div className={`text-center p-3 rounded border border-gray-600 ${bgClass}`}>
      <div className={`font-semibold ${textClass}`}>{label}</div>
      <div style={{ color: teamColor }}>{value.toFixed(2)}</div>
    </div>
  );
}

function DistortionCoefficientsCard({ data }: { data: typeof calibrationData.resolutionResults[number] }) {
  return (
    <Card className="bg-zinc-900 border-gray-700" style={{ color: teamColor }}>
      <CardHeader>
        <CardTitle>Distortion Coefficients - {data.resolution}</CardTitle>
        <CardDescription>Radial and tangential distortion parameters</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          <div className="grid grid-cols-1 gap-3">
            {Object.entries(data.distortionCoefficients).map(([key, value]) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <Badge variant="outline" className="font-mono border-gray-500 text-gray-300">
                    {key}
                  </Badge>
                  <span className="text-sm">{key.startsWith("k") ? "Radial" : "Tangential"}</span>
                </div>
                <span className="font-mono font-semibold">{value.toFixed(6)}</span>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <Card className="bg-zinc-900 border-gray-700" style={{ color: teamColor }}>
      <CardHeader>
        ...
        <CardDescription>Compare focal lengths and principal points</CardDescription>
      </CardHeader>
      <CardContent>
        <Table className="border-gray-700">
          <TableHeader>
            <TableRow className="border-gray-700 hover:bg-">
          ...
          <TableBody>
            {resolutions.map((result, index) => (
              ...
              >
                ...
                <TableCell className="font-mono">{result.cameraMatrix.fx.toFixed(2)}</TableCell>
                <TableCell className="font-mono">{result.cameraMatrix.fy.toFixed(2)}</TableCell>
                <TableCell className="font-mono">{result.cameraMatrix.cx.toFixed(2)}</TableCell>
                <TableCell className="font-mono">{result.cameraMatrix.cy.toFixed(2)}</TableCell>
...
export default function CameraCalibrationPage() {
  const [selectedResolutionData, setSelectedResolutionData] = useState(calibrationData.resolutionResults[0])
  const [collapsed, setCollapsed] = useState(false)

  return (
    <div className="max-w-[1100px] min-h-screen p-2" style={{ color: teamColor }}>
      <div
        className="space-y-6"
        style={{ backgroundColor: baseCardColor, borderRadius: "12px", padding: "24px" }}
      >
        <CalibrationHeader
        ...
        <div
          className={`overflow-hidden transition-all
                      ...
                      ${collapsed ? "max-h-0 opacity-0 p-0" : "max-h-[2000px] opacity-100 p-4 space-y-2"}`}
          style={{ color: teamColor }}
        >
          <ResolutionSelector
